        ...
        foreach ($jobPosts as $index => $jobPost) {
            ...
            foreach ($scoresByApplicant as $rows) {
                ...
                if (!$first->nPersonalInfo_id && $first->ControlNo) {
                    ...
                    // ── Outsider: only show info if a CURRENT/PRESENT qualified work experience exists ──
                    $qualifiedIds = [];
                    ...
                    if (!empty($qualifiedIds)) {
                        ...
                        // ── Find entries where work_date_to is blank, "current", or "present" (case-insensitive) ──
                            $currentEntries = $workExperiences->filter(function ($w) {
                                $toRaw = strtolower(trim($w->work_date_to ?? ''));
                                return str_contains($toRaw, 'current') || str_contains($toRaw, 'present');
                            }); 
